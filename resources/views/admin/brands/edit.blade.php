        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <div class="d-flex align-items-center gap-2">
                    <h1 class="page-header-title mb-0 fs-3 fw-bold">{{ $brand->name }}</h1>
                ...
                <p class="page-header-subtitle text-muted mb-0 small">Update brand details, official website, logo, and visibility status.</p>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="card-panel border-0 shadow-sm rounded-4 p-4 bg-white">
            <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-section mb-4">
                    ...
                    <div class="mt-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="url" name="website" id="website" class="form-control border-start-0 rounded-end-3 p-2.5 @error('website') is-invalid @enderror" 
                                   value="{{ old('website', $brand->website) }}" placeholder="https://www.nvidia.com">
                        </div>
                        @error('website')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mt-3">
                        ...
                        <textarea name="description" id="description" rows="3" class="form-control rounded-3 p-2.5 @error('description') is-invalid @enderror" 
                                  placeholder="Briefly describe the manufacturer products and specialty...">{{ old('description', $brand->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row g-3 align-items-center">
                        <div class="col-md-6">
                            ...
                            @if($brand->logo)
                                <div class="d-flex align-items-center mb-2">
                                    <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" style="width: 44px; height: 44px; object-fit: contain;" class="me-2 border p-1 rounded-2 bg-white">
                            ...
                            <div class="form-text small">Choose new image to replace current logo.</div>
                            @error('logo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="sort_order" id="sort_order" class="form-control rounded-3 p-2.5 @error('sort_order') is-invalid @enderror" 
                                   value="{{ old('sort_order', $brand->sort_order) }}" min="0" style="max-width: 160px;">
                    ...
                    <div class="row g-3 align-items-center">
                        <div class="col-md-6">
                            ...
                            <div class="form-check form-switch p-0 ms-4">
                                <input class="form-check-input" type="checkbox" role="switch" id="is_active" name="is_active" value="1" {{ old('is_active', $brand->is_active) ? 'checked' : '' }} style="width: 48px; height: 24px; cursor: pointer;">
                                <label class="form-check-label fw-semibold ms-3 pt-1" for="is_active" id="status-label">
                                    {{ old('is_active', $brand->is_active) ? 'Active (Visible in catalog)' : 'Inactive (Hidden)' }}
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-check form-switch p-0 ms-4">
                                <input class="form-check-input" type="checkbox" role="switch" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $brand->is_featured) ? 'checked' : '' }} style="width: 48px; height: 24px; cursor: pointer;">
                ...
                <div class="pt-4 border-top d-flex align-items-center gap-3">
                    <button type="submit" class="btn-primary-solid ripple-btn px-4 py-2.5 rounded-3 fw-semibold" id="btn-update-brand">
        ...
        <div class="sticky-top" style="top: 90px;">
            <div class="card-panel border-0 shadow-sm rounded-4 p-4 bg-white mb-4">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    ...
                    <span class="badge bg-warning-subtle text-amber border rounded-pill px-2.5 py-1 small {{ $brand->is_featured ? '' : 'd-none' }}" id="preview-featured-badge">
                        ★ Featured
                    </span>
                </div>
                ...
                <div class="brand-preview-card p-4 rounded-4 text-center border position-relative overflow-hidden" style="background: linear-gradient(180deg, #f8fafc 0%, #ffffff 100%);">
                    ...
                    <div class="position-absolute top-0 end-0 m-3">
                        <span class="badge {{ $brand->is_active ? 'bg-success' : 'bg-secondary' }} rounded-pill px-2.5 py-1" id="preview-status-pill">
                    ...
                    <div class="mx-auto mb-3 shadow-sm rounded-4 d-flex align-items-center justify-content-center bg-white border font-monospace fw-bold text-primary" id="preview-logo-box" style="width: 80px; height: 80px; font-size: 1.8rem;">
                        @if($brand->logo)
                            <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" style="width: 100%; height: 100%; object-fit: contain; padding: 6px;">
                        @else
                    ...
                    <p class="text-muted small mb-2 font-monospace" id="preview-slug">/brand/{{ $brand->slug }}</p>
                    ...
                    <div class="pt-3 border-top d-flex align-items-center justify-content-between small text-muted">
                        <span id="preview-website"><i class="bi bi-globe me-1"></i> {{ $brand->website ? parse_url($brand->website, PHP_URL_HOST) : 'website.com' }}</span>
                        <span>Sort: <strong class="text-dark" id="preview-sort">{{ $brand->sort_order }}</strong></span>
                    </div>
                </div>
            </div>
            ...
            {{-- Quick Action Card --}}
            <div class="card-panel border-0 shadow-sm rounded-4 p-4 bg-white">
                <h6 class="fw-bold text-dark mb-2">Brand Actions</h6>
                <p class="small text-muted mb-3">Permanently remove this brand from your catalog.</p>

                <form action="{{ route('admin.brands.destroy', $brand) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete brand \'{{ $brand->name }}\'?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-outline-danger w-100 rounded-3 py-2 fw-semibold btn-sm">
                        <i class="bi bi-trash3-fill me-1"></i> Delete Brand
                    </button>
                </form>
            </div>
        </div>
